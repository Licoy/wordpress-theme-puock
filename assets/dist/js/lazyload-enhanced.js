...
class PuockLazyLoadEnhanced {
    ...
    loadImage(img) {
        ...
        // 验证 src 是否有效
        if (!src || src === 'null' || src === '' || src.includes('null') || src.includes('^/')) {
            console.warn('Invalid data-src detected:', src, img);
    ...
    onImageLoaded(img, tempImg, src, srcset) {
        // 最终验证 - 确保不会设置 null 值
        if (!src || src === 'null' || src.includes('null') || src.includes('^/')) {
            console.error('Attempted to set invalid src:', src);
            this.onImageError(img, src, srcset);
            return;
        }
    ...
    progressiveLoad(img, src, srcset) {
        // 验证 src
        if (!src || src === 'null' || src.includes('null') || src.includes('^/')) {
            console.error('Invalid src in progressiveLoad:', src);
            return;
        }
