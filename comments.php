    ...
    <div class="mt20 clearfix" id="comment-form-box">
        <form class="mt10" id="comment-form" method="post">
            <div class="form-group">
                <textarea placeholder="<?php _e('您必须要登录之后才可以进行评论', PUOCK) ?>" disabled id="comment" name="comment" class="form-control form-control-sm t-sm" rows="1"></textarea>
            </div>
        </form>
        <?php if(pk_is_checked('oauth_qq')): ?>
        <div>
            <a data-no-instant href="<?php echo pk_oauth_url_page_ajax('qq',get_the_permalink()) ?>" class="btn btn-danger btn-ssm ahfff"><i class="czs-qq"></i>&nbsp;<?php _e('QQ登录', PUOCK) ?></a>
        </div>
        <?php endif; ?>
    </div>
    <?php else: ?>
    <div class="mt20 clearfix" id="comment-form-box">
        <form class="mt10" id="comment-form" method="post" action="<?php echo admin_url().'admin-ajax.php?action=comment_ajax' ?>">
            ...
            <div class="row row-cols-1 comment-info">
                ...
                <div class="col-12 col-sm-4"><input type="text" id="author" name="author" class="form-control form-control-sm t-sm" placeholder="<?php _e('昵称（必填）', PUOCK) ?>"></div>
                <div class="col-12 col-sm-4"><input type="email" id="email" name="email" class="form-control form-control-sm t-sm" placeholder="<?php _e('邮箱（必填）', PUOCK) ?>"></div>
                <div class="col-12 col-sm-4"><input type="text" id="url" name="url" class="form-control form-control-sm t-sm" placeholder="<?php _e('网站', PUOCK) ?>"></div>
                <?php endif; ?>
            </div>
            ...
            <div class="clearfix mt10">
                <div class="float-left">
                    ...
                    <div class="puock-text t-sm">
                        ...
                        <span><strong><?php echo $user->data->display_name ?></strong>，<a data-no-instant class="ta3 a-link" href="<?php echo wp_logout_url(get_the_permalink()) ?>"><?php _e('登出', PUOCK) ?></a></span>
                    </div>
                    <?php endif; ?>
                ...
                <div class="float-right">
                    <button id="comment-cancel" type="button" class="btn btn-outline-dark d-none btn-ssm"><?php _e('取消', PUOCK) ?></button>
                    <button id="comment-smiley" class="btn btn-outline-secondary btn-ssm" type="button"><i class="czs-face-l t-md"></i></button>
                    <button id="comment-submit" type="submit" class="btn btn-primary btn-ssm"><i class="czs-paper-plane-l"></i>&nbsp;<?php _e('发布评论', PUOCK) ?></button>
                </div>
            </div>
        </form>
    </div>
